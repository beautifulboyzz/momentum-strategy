...
def load_data_and_calc_atr(folder, atr_window=20):
    ...
    if not price_dict:
        return None, None, None, "未读取到有效数据"
...
def run_strategy_logic(df_prices, df_vols, df_lows, params):
    """
    核心策略逻辑：完全复刻第二个代码 (dual momentum.py) 的逻辑
    """
    # 解包参数
    ...
    filter_ma = params['ma']
    stop_loss_pct = params['stop_loss_pct']  # 新增止损参数
    ...
    # 运行时状态变量
    weights = {}  # 目标持仓权重
    curr_holdings = {}  # 实际持仓权重 (会因止损变为0)
    entry_prices = {}  # 记录入场价/昨日价用于计算止损
    ...
    for i in range(start_idx_loc, len(full_dates)):
        ...
        # 1. 每日选股 (Daily Rebalance) - 复刻代码2逻辑
        try:
            ...
            # 更新成本参考价 (简化处理：每日调仓模式下，参考价即为昨日收盘价)
        ...
        if res_nav.empty:
            ...
            with tab_chart:
                ...
                fig.add_trace(go.Scatter(
                    x=plot_data.index,
                    y=plot_data['return_pct'],
                    mode='lines',
                    name='累计收益率',
                    line=dict(color='#ff7f0e', width=2.5),  # 使用 Code 2 的橙色风格
                    fill='tozeroy',
            ...
            with tab_attr:
                ...
                fig_bar = px.bar(res_contrib, x='Contribution', y='Asset', orientation='h',
                                 ...
                                 color_continuous_scale=['green', '#f0f2f6', 'red'])  # 绿跌红涨
